...
本项目是一个基于 JavaFX 的 Windows 本地开发工具箱，集成进程与端口查看、开发环境体检、网络诊断、文件分析、Base64 编解码、HTTP/WebSocket 调试、应用与 JAR 管理、备忘提醒、域名保活和电源计划。界面采用浅色分组工作区与按需加载机制，长期驻留时减少无效轮询和后台占用。
...
- BIOS / 电源检测页只读展示整机、主板、BIOS、UEFI、睡眠状态、唤醒设备、唤醒计时器和厂商管理接口
